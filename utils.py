...
def control_vector_dataset(
    data_path: str, 
    tokenizer: PreTrainedTokenizer, 
    user_tag: str = "", 
    assistant_tag: str = "", 
    control_vector: str = "honest",  # Generalized control vector parameter
    opposite_vector: str = "untruthful",  # Opposite of the control vector
    seed: int = 0
) -> (list, list):
    """
    Processes data to create training and testing datasets based on a control vector.

    Args:
    - data_path (str): Path to the CSV containing the data.
    - tokenizer (PreTrainedTokenizer): Tokenizer to tokenize statements.
    - user_tag (str): User template for instruction.
    - assistant_tag (str): Assistant template for instruction.
    - control_vector (str): Type of behavior or quality being modeled (e.g., 'honest').
    - opposite_vector (str): Opposite of the control vector (e.g., 'untruthful' for honesty).
    ...
    # Generalized template for any control vector
    ...
    for statement in true_statements:
        ...
        for idx in range(1, len(tokens) - 5):
            ...
            control_statements.append(f"{user_tag} {template_str.format(type=control_vector)} {assistant_tag} " + truncated_statement)
            opposite_statements.append(f"{user_tag} {template_str.format(type=opposite_vector)} {assistant_tag} " + truncated_statement)

    # Create training data
    ntrain = 512
    combined_data = [[control, opposite] for control, opposite in zip(control_statements, opposite_statements)]
    ...
    # Create test data
    reshaped_data = np.array([[control, opposite] for control, opposite in zip(control_statements[:-1], opposite_statements[1:])]).flatten()
